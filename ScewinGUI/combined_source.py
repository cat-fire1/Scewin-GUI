...
try:
    ...
    from PyQt6.QtGui import QColor, QIcon, QDesktopServices, QAction, QRegularExpressionValidator, QBrush, QKeySequence, QShortcut, QPixmap, QCursor
    from PyQt6.QtWidgets import (
        QApplication, QWidget, QHBoxLayout, QVBoxLayout, 
        QHeaderView, QFileDialog, QTableWidgetItem, QFrame, QMenu,
        QAbstractItemView, QLabel, QToolTip
    )
...
class BiosService:
    ...
    def check_dependencies(self) -> Tuple[bool, List[str]]:
        # Verify required SCEWIN files are present in the working folder.
        required = [self.scewin_path, "amifldrv64.sys", "amigendrv64.sys"]
    ...
    def create_backup(self, source: str, backup_dir: str = "backups") -> str:
        ...
        dest = os.path.join(backup_dir, f"nvram_backup_{timestamp}.txt")
...
CONFIG_FILE: str = os.path.join(os.path.dirname(os.path.abspath(sys.argv[0])), "settings.ini")
...
class NVRAMInterface(QWidget):
    ...
    def check_dependencies(self) -> None:
        ok, missing = self.service.check_dependencies()
        if not ok:
            msg = self.t("deps.missing", files=", ".join(missing))
    ...
    def load_data(self, filepath):
        ...
        try:
            content = self.service.load_file(filepath)
            self.settings = self.parser.parse_file(content)
            self.populate_table()
    ...
    def action_import_bios(self):
        if not os.path.exists(NVRAM_NEW_FILE):
            InfoBar.error(
    ...
    def run_worker(self, cmd: str, args: List[str], desc: str) -> None:
        if self.worker is not None:
            if self.worker.isRunning():
                ...
                self.worker.wait(WORKER_WAIT_MS)
            try:
        ...
        self.worker = BiosWorker(cmd, args, desc)
